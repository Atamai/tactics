/*=========================================================================
  ...
  Module:    vtkCornerAnnotation2.cxx
...
=========================================================================*/
#include "vtkCornerAnnotation2.h"
...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkCornerAnnotation2);

vtkSetObjectImplementationMacro(vtkCornerAnnotation2,ImageActor,vtkImageActor);
vtkSetObjectImplementationMacro(vtkCornerAnnotation2,WindowLevel,
                                vtkImageMapToWindowLevelColors);
vtkCxxSetObjectMacro(vtkCornerAnnotation2,TextProperty,vtkTextProperty);

//----------------------------------------------------------------------------
vtkCornerAnnotation2::vtkCornerAnnotation2()
{
...
void vtkCornerAnnotation2::ReleaseGraphicsResources(vtkWindow *win)
{
...
//----------------------------------------------------------------------------
void vtkCornerAnnotation2::TextReplace(vtkImageActor *ia,
...
//----------------------------------------------------------------------------
int vtkCornerAnnotation2::RenderOverlay(vtkViewport *viewport)
{
...
//----------------------------------------------------------------------------
int vtkCornerAnnotation2::RenderOpaqueGeometry(vtkViewport *viewport)
{
  int fontSize;
  int i;
  ...
  if (viewport_size_has_changed ||
    ...
    if (viewport_size_has_changed ||
      ...
      if (tprop_has_changed)
        ...
        tprop = this->TextMapper[4]->GetTextProperty();
        tprop->ShallowCopy(this->TextProperty);
        tprop->SetFontSize(fontSize);
        tprop->SetColor(1.0,1.0,0.0);          
          
        tprop = this->TextMapper[5]->GetTextProperty();
        tprop->ShallowCopy(this->TextProperty);
        tprop->SetFontSize(fontSize);
        tprop->SetColor(1.0,1.0,0.0);
      ...
      */
      ...
      int height_02 = tempi[1] + tempi[5] + tempi[9];
      int height_13 = tempi[3] + tempi[7] + tempi[11];
      ...
      int num_lines_02 = 
        ...
        this->TextMapper[4]->GetNumberOfLines();

      int num_lines_13 = 
        this->TextMapper[1]->GetNumberOfLines() + 
        this->TextMapper[3]->GetNumberOfLines() +
        this->TextMapper[5]->GetNumberOfLines();
      ...
      while (height_02 < tSize[1] && 
        ...
        height_02 = tempi[1] + tempi[5] + tempi[9];
        height_13 = tempi[3] + tempi[7] + tempi[11];
...
int vtkCornerAnnotation2::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//----------------------------------------------------------------------------
void vtkCornerAnnotation2::SetTextActorsPosition(int vsize[2])
{
  ...
  this->TextActor[4]->SetPosition(5, 0.5*vsize[1]);
  this->TextActor[5]->SetPosition(vsize[0] - 5, 0.5*vsize[1]);
}
      
//----------------------------------------------------------------------------
void vtkCornerAnnotation2::SetTextActorsJustification()
{
  vtkTextProperty *tprop = this->TextMapper[0]->GetTextProperty();
...
//----------------------------------------------------------------------------
void vtkCornerAnnotation2::SetText(int i, const char *text)
{
  if (i < 0 || i > 5)
    {
    return;
...
//----------------------------------------------------------------------------
const char* vtkCornerAnnotation2::GetText(int i)
{
  if (i < 0 || i > 5)
    {
    return NULL;
...
//----------------------------------------------------------------------------
void vtkCornerAnnotation2::RemoveText(int i)
{
  if (i < 0 || i > 5)
  {
    return;
...
//----------------------------------------------------------------------------
void vtkCornerAnnotation2::ClearAllTexts()
{
  for (int i = 0; i < 6; i++)
    {
    this->SetText(i, "");
...
//----------------------------------------------------------------------------
void vtkCornerAnnotation2::CopyAllTextsFrom(vtkCornerAnnotation2 *ca)
{
  for (int i = 0; i < 6; i++)
    {
...
//----------------------------------------------------------------------------
void vtkCornerAnnotation2::PrintSelf(ostream& os, vtkIndent indent)
{
